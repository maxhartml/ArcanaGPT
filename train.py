...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # calculate query, key, values for all the heads in batch and move head forward to be the batch
        # nh is "number of heads", hs is "head size", and C (number of channels) = nh * hs
        # e.g. in GPT-2 (124M), h_heads=12, hs=64, so nh*hs=768 channels in the Transformer
        qkv = self.c_attn(x)
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        x = self.c_fc(x)   # Linear transformation: (B, T, n_embd) -> (B, T, 4 * n_embd)
        x = self.gelu(x)   # Non-linear activation: GELU
        x = self.c_proj(x) # Linear transformation: (B, T, 4 * n_embd) -> (B, T, n_embd)
        return x
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        # idx is of shape (B, T)
        B, T = idx.size()
        assert T <= self.config.block_size, f'Cannot forward sequence of length {T}, block size is maximum'
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        # i.e. all weight tensors in matmuls + embedding decay will, all biases and laynorms will  not.
        ...
        # create AdamW optimizer and use the fused version if available (i.e. have 'cuda') - speeds up training
...
# torchrun command sets the env variables RANK, LOCAL_RANK, and WOLRD_SIZE
ddp = int(os.environ.get('RANK', -1)) != -1 # is this a ddp run?
if ddp:
    # use of DDP atm demands CUDA, we set the device appropiately according to rank
...
for step in range(max_steps):
    ...
    if step % 100 == 0:
        ...
        if master_process:
            ...
            if step > 0 and (step % 5000 == 0 or last_step):
                ...
                # might also want to add optimiser.state_dict() and rng seeds etc., if wanted to more exactly resume training
    ...
    if ((step > 0 and step % 250 == 0) or last_step) and (not use_compile):
        ...
        while xgen.size(1) < max_length:
            ...
            with torch.no_grad():
                ...
                # gather the corresponding incices
    ...
    for micro_step in range(grad_accum_steps):
        ...
        # addition of gradients corresponds to a SUM in the objective, but instead of a SUM we want MEAN. Scale the loss here so it comes out right.
